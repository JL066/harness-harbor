...
def _wait_for_worker_exit(proc):
    from harbor_platform.process import wait_owned_tree_exit

    exit_code = proc.wait(timeout=5)
    assert wait_owned_tree_exit(proc, timeout=5), "Fixture process tree did not exit"
    return exit_code
...
class TestCodexJobDaemon(unittest.TestCase):
    ...
    def test_codex_3_concurrency_and_4th_queued(self) -> None:
        ...
        self.assertEqual(_wait_for_worker_exit(scheduler.active_workers["codex"][j1.name].proc), 0)
    ...
    def test_worker_failure_isolation(self) -> None:
        ...
        self.assertEqual(_wait_for_worker_exit(scheduler.active_workers["codex"][j_crash.name].proc), 42)
        scheduler.tick()
    ...
    def test_acceptance_parallel_scheduling_timeline(self) -> None:
        ...
        self.assertEqual(_wait_for_worker_exit(scheduler.active_workers["codex"][codex_jobs[0].name].proc), 0)
        ...
        for workers in scheduler.active_workers.values():
            for worker in workers.values():
                self.assertEqual(_wait_for_worker_exit(worker.proc), 0)
