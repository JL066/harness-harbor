...
@pytest.mark.skipif(sys.platform != "win32", reason="Windows API binding")
def test_windows_backend_binds_advapi_with_last_error_support(monkeypatch):
    ...
    monkeypatch.setattr(credential_store.ctypes, "WinDLL", fake_windll)
...
def test_macos_keychain_backend_stub_fails_closed():
    """macOS Keychain stub fails closed until implemented in a future batch."""
    mac_backend = MacOSKeychainBackend()

    with pytest.raises(CredentialBackendUnavailableError):
        mac_backend.store("target", "secret")

    with pytest.raises(CredentialBackendUnavailableError):
        mac_backend.read("target")

    with pytest.raises(CredentialBackendUnavailableError):
        mac_backend.delete("target")

    with pytest.raises(CredentialBackendUnavailableError):
        mac_backend.exists("target")
