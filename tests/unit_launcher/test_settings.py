...
def test_config_path_returns_under_appdata_by_default(monkeypatch):
    ...
    appdata = Path(r"C:\Users\test\AppData\Roaming")
    monkeypatch.setenv("APPDATA", str(appdata))
    settings.reload()
    assert settings.config_path() == appdata / "harbor-launcher" / "config.yaml"
...
def test_config_derived_paths_follow_production_path(monkeypatch):
    ...
    assert reloaded.TUNNEL_LOG == reloaded.PRODUCTION_PATH / "tunnel-supervisor.log"
    assert reloaded.DAEMON_LOG == reloaded.PRODUCTION_PATH / "codex-job-daemon.log"
    assert reloaded.START_TUNNEL_SCRIPT == reloaded.PRODUCTION_PATH / "start-tunnel.ps1"
    assert reloaded.START_DAEMON_SCRIPT == reloaded.PRODUCTION_PATH / "start-codex-job-daemon.ps1"
