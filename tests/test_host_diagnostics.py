...
class TestHostDiagnosticsPlatformSupport(unittest.TestCase):
    """Windows-only diagnostics must report unsupported platforms explicitly."""

    def test_windows_diagnostics_are_unsupported_on_macos(self) -> None:
        with mock.patch.object(host_diagnostics, "IS_WINDOWS", False), mock.patch.object(
            host_diagnostics.sys, "platform", "darwin"
        ):
            results = {
                "port_listeners": host_diagnostics.port_listeners(5173),
                "process_inspect": host_diagnostics.process_inspect(12345),
                "firewall_query": host_diagnostics.firewall_query(port=5173),
                "network_interfaces": host_diagnostics.network_interfaces(),
            }
...
class TestFirewallQueryLogic(unittest.TestCase):
    ...
    @mock.patch.object(host_diagnostics, "IS_WINDOWS", True)
...
class TestAsyncServerLegacyTools(unittest.IsolatedAsyncioTestCase):
    ...
    @mock.patch.object(host_diagnostics, "IS_WINDOWS", True)
    @mock.patch("host_diagnostics.run_safe_subprocess")
    async def test_async_port_listeners(self, mock_subp: mock.MagicMock) -> None:
        mock_subp.return_value = subprocess.CompletedProcess(
            args=["netstat.exe", "-ano"],
            returncode=0,
            stdout="TCP 0.0.0.0:5173 0.0.0.0:0 LISTENING 1234",
            stderr="",
        )
