...
def spawn_detached_supervisor(script_path: str) -> bool:
    ...
    try:
        ...
        from launcher.credential_store import CredentialStore, CREDENTIAL_TARGET_CODEX_CUSTOM_API_KEY, CREDENTIAL_TARGET_TUNNEL_RUNTIME_KEY
        is_tunnel = str(script_path) == str(START_TUNNEL_SCRIPT)
        current = load_user_settings()
        env = child_environment(current.to_dict(), tunnel=is_tunnel)
        store = CredentialStore()
        custom_key = store.read(CREDENTIAL_TARGET_CODEX_CUSTOM_API_KEY)
        if custom_key:
            env["HARBOR_CODEX_CUSTOM_API_KEY"] = custom_key
        if is_tunnel:
            tunnel_key = store.read(CREDENTIAL_TARGET_TUNNEL_RUNTIME_KEY)
            if tunnel_key:
                env["TUNNEL_RUNTIME_KEY"] = tunnel_key
        subprocess.Popen(
            cmd,
            env=env,
            creationflags=flags,
