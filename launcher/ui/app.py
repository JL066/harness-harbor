...
class HarborLauncherApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        self._slow_probe_thread = None
        self._slow_probe_result = ([], [])
    ...
    def _run_health_poll_async(self):
        ...
        if self._slow_probe_thread is None or not self._slow_probe_thread.is_alive():
            def slow_probe():
                try:
                    telemetry = harness_telemetry_snapshot()
                    self._slow_probe_result = (list_harnesses(telemetry_snapshot=telemetry), agy_models(telemetry_snapshot=telemetry))
                except Exception:
                    self._slow_probe_result = ([], [])
            self._slow_probe_thread = threading.Thread(target=slow_probe, daemon=True)
            self._slow_probe_thread.start()
        ...
        def worker():
            try:
                ...
                rows, models = self._slow_probe_result
                connected = snapshot.mcp.status in {"Healthy", "Running"}
                harness_statuses = harness_snapshot(JOBS_DIR, rows, connected=connected)
                self.after(0, self._apply_harness_statuses, harness_statuses, models)
            except Exception as e:
                from harbor_runtime.snapshot import harness_snapshot
                from control_plane import JOBS_DIR
                self.after(0, self._apply_harness_statuses, harness_snapshot(JOBS_DIR, connected=False), [])
            finally:
