...
def first_run_status(
    ...
    *,
    ...
    try:
        ...
        if not store.exists(conn.credential_ref or CREDENTIAL_TARGET_TUNNEL_RUNTIME_KEY):
            return FirstRunStatus(True, "Tunnel Runtime Key is not configured.")
        custom = current.codex.custom
        if current.codex.routing_mode == "custom" or custom.enabled:
            if not custom.profile_name.strip() or not custom.base_url.strip():
                return FirstRunStatus(True, "Custom Codex route details are incomplete.")
            if not store.exists(custom.credential_ref or CREDENTIAL_TARGET_CODEX_CUSTOM_API_KEY):
                return FirstRunStatus(True, "Custom Codex API key is not configured.")
    except Exception:
        # Secure-store errors fail closed: setup remains required.
...
def validate_draft(draft: dict) -> list[str]:
    ...
    if (err := _url(str(conn.get("base_url", "")), "Tunnel base URL")):
        errors.append(err)
    profile = str(conn.get("profile_name", "")).strip()
    if profile and any(not (c.isalnum() or c in "-_.") for c in profile):
        errors.append("Profile name may contain only letters, numbers, '-', '_' and '.'.")
    codex = draft.get("codex", {})
    route = str(codex.get("routing_mode", "current")).strip().lower()
    if route not in ROUTE_CHOICES:
        errors.append("Choose a supported Codex route.")
    custom = codex.get("custom", {})
    if route == "custom" or bool(custom.get("enabled")):
        if not str(custom.get("profile_name", "")).strip():
            errors.append("Custom provider profile name is required.")
        if (err := _url(str(custom.get("base_url", "")), "Custom provider base URL")):
            errors.append(err)
    return errors
