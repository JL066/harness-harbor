...
class TunnelProfileManager:
    ...
    def validate(self) -> None:
        ...
        if not _safe_name(self.profile_name):
            raise TunnelConfigurationError("Invalid tunnel profile name.")
        if not conn.tunnel_id or not isinstance(conn.tunnel_id, str):
            raise TunnelConfigurationError("Tunnel ID is required.")
        parsed = urlparse(conn.base_url)
        if parsed.scheme not in ("http", "https") or not parsed.netloc:
            raise TunnelConfigurationError("Tunnel base URL must be an HTTP(S) URL.")
    ...
    def render(self) -> str:
        ...
        command = subprocess.list2cmdline([str(self.python_path), str(self.mcp_entrypoint)])
...
class ManagedTunnelSupervisor:
    ...
    def start(self) -> Any:
        ...
        try:
            self._process = self._popen_factory(cmd, env=env, close_fds=True, creationflags=flags)
        except Exception:
            self._process = None
            raise
        return self._process
